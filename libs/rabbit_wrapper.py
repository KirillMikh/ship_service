...
import rabbitpy
RABITMQ_URL= 'amqp://ypmuorzj:hyrxWh_HrqpbllRip6h8htCc7RRkZTej@chinook.rmq.cloudamqp.com/ypmuorzj'
class RabbitQueue:
...
if __name__ == '__main__':
    ...
    #насколько понял из админки реббита запись работает быстрее чем чтение,
    # конкретно для этого примера примерно в полтора раза
    #сначала работает запись только потом чтение, одновременно не получается сделать. Обсуждали эту проблему на занятии.
